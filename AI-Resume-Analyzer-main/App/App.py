# Developed by Nida, Iqra, Moneeba    Made with Streamlit
...
def run():
    ...
    img = Image.open('./Logo/group_logo.png')
    st.image(img)
    st.sidebar.markdown("# Choose Something...")
    activities = ["User", "Feedback", "About", "Admin"]
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    link = '<b>Built with 🤍 by Group Project - Nida, Iqra, Moneeba</b>' 
    ...
    # Create the DB (SQLite handles this automatically, but we ensure tables exist)
    ...
    admin_table_sql = "CREATE TABLE IF NOT EXISTS admin_users (username TEXT PRIMARY KEY, password TEXT);"
    ...
    ###### CODE FOR ABOUT PAGE ######
        ...
        <p align="justify">
            Built with 🤍 by Group Project - Nida, Iqra, Moneeba
        </p>
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        # Admin Choice for Login or Register
        admin_option = st.selectbox("Choose Action", ["Login", "Create Admin"])

        if admin_option == 'Create Admin':
            ...
            if st.button('Create'):
                if new_user and new_password:
                    try:
                        cursor.execute("INSERT INTO admin_users (username, password) VALUES (?, ?)", (new_user, new_password))
                        connection.commit()
                        st.success("Admin account created successfully!")
                    except sqlite3.IntegrityError:
                        st.error("Admin already exists with this username.")
                else:
            ...
            if st.button('Login'):
                ...
                cursor.execute("SELECT * FROM admin_users WHERE username = ? AND password = ?", (ad_user, ad_password))
                res = cursor.fetchone()

                ## Credentials Check (Database or Legacy Hardcoded)
                if res or (ad_user == 'admin' and ad_password == 'admin@resume-analyzer'):
